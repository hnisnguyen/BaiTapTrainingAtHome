...
if (isset($_POST['delete'])) {
    ...
    if ($result1 == 1) {
        $result2 = deleteContent($id_menu);
        if ($result2 == 1) {
            echo("<meta http-equiv='refresh' content='0; url=list.php?table=$table&parent=$parent'>"); // Trả về trang list
        }
    }
}
?>
<?php if (isset($_GET['id']) && isset($_GET['table']) && isset($_GET['parent'])) {
    ...
    $parent = $_GET['parent'];
    $id_menu = $_GET['id']; ?>
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?table=$table&parent=$parent&id=$id_menu")  ?>" method="post">
        ...
        <div id="popup-del-lesson" class="overlay">
            <div class="popup">
                <a class="close" href='list.php?table=<?= $table ?>&parent=<?= $parent ?>'>&times;</a>
                <div class="content">
